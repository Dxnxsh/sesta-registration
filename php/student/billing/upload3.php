...
if (!$id) {
    SecuritySanitizer::logSecurityEvent('Invalid session ID in upload3.php', 'HIGH');
    header("Location: ../login-logout/login.php");
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES["file"]) && $_FILES["file"]["error"] == 0) {
        ...
        // Check file size (max 5MB for PIBG fee receipts)
        ...
        $uniqueFilename = uniqid('pibg_receipt_', true) . '.' . $fileExtension;
        ...
        if (move_uploaded_file($tmpName, $targetFile)) {
            ...
            $stmt = $con->prepare("UPDATE payment SET PAYMENT_RECEIPT = ?, PAYMENT_STATUS = 'PENDING' WHERE STUDENT_ID = ? AND PAYMENT_TYPE = 'PIBG FEES'");
            $stmt->bind_param("ss", $uniqueFilename, $res_IC);
            
            if ($stmt->execute()) {
                SecuritySanitizer::logSecurityEvent("PIBG fee receipt uploaded successfully for student: $res_IC", 'INFO');
                $stmt->close();
